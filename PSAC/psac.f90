...
!---------------!
    ...
    real(8) :: avx(0:2) ! average chi2
    real(8) :: ava(0:2) ! average amplitude of main feature
    real(8) :: avw(0:2) ! average frequency of main feature
...
!================!
    ...
    if (pf==0) then
        write(10,'(1ES16.8)') avx(0)/nt
    else
        write(10,'(2ES16.8)') avx(0)/nt, avw(0)
    endif
    close(10)
    enddo
...
!-----------------------------!
    ...
    write(10,'(i6,7ES16.8)') i, log(th), avx(1:2)/nt, ava(1:2), avw(1:2)
    close(10)
    it(1,i)=th
    it(2,i)=avx(1)
    dt(:,i)=del(:)
...
!----------------------------!
    ...
    write(10,'(5ES16.8)') a0, avx(1:2)/nt, avw(1:2)
    close(10)    
    if (avx(1)>ax+2.d0*dble(nt)) then
        exit
    elseif (avx(1)<ax) then
        ax=avx(1)
        wp=ww
        ap=aw
        del1=del
    ...
    open(10,file='log.log',position='append') 
    write(10,*)'Sampling at final A(0) = ',a0
    close(10)
    call equilibrate(0,stps/5,5,del)
    call writeconf()
...
!-----------------------------!
    ...
    if (bins==0) then
        ...
        avx(2)=sqrt((avx(2)-avx(1)**2)/dble(bins))
        if (pf==1) then
            ava(1:2)=ava(1:2)/dble(bins)
            avw(1:2)=avw(1:2)/dble(bins)
            ava(2)=sqrt((ava(2)-ava(1)**2)/dble(bins))    
            avw(2)=sqrt((avw(2)-avw(1)**2)/dble(bins))    
        endif
    endif
...
!----------------------------!
    ...
    open(10,file='log.log',position='append') 
    if (pf==0) then
        write(10,'(2i4,2ES12.4,a,2ES12.4,a,2ES12.4, ES12.4)') a, b, x0/nt, avx(0)/nt, ' | ', acr(1:2), ' | ', del(1:2)*dw, th
    else
        write(10,'(2i4,2ES12.4,a,3ES12.4,a,3ES12.4, ES12.4)') a, b, x0/nt, avx(0)/nt, ' | ', acr(1:3), ' | ', del(1:3)*dw, th
    endif
    close(10)
...
!----------------------!
    ...
    write(10,*)nw
    do i=1-pf,nw
    write(10,'(i9,3ES16.8)') ww(i), dble(ww(i))*dw, aw(i)
    enddo
    close(10)
